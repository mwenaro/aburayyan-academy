...
const {students, fetchStudents} = useStudentStore()
  ...
  const renderRow = (item:any) => (
    <tr key={item._id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight">
      ...
      <td className="hidden md:table-cell">{item.class.grade}</td>
      <td className="hidden md:table-cell">{item.contactDetails.phone}</td>
      <td className="hidden md:table-cell">{item.address.town}, {item.address.county}</td>
      <td>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end">
            <Button className="w-8 h-8 flex items-center justify-center rounded-full bg-lamaYellow">
              <Image src="/u-dashboard/filter.png" alt="Filter" width={14} height={14} />
            </Button>
            <Button className="w-8 h-8 flex items-center justify-center rounded-full bg-lamaYellow">
              <Image src="/u-dashboard/sort.png" alt="Sort" width={14} height={14} />
            </Button>
