...
import { RegistrationInvitationModel, IRegistrationInvitation } from "@/models/RegistrationInvite";
...
export class RegistrationInviteManager {
     ...
     */
    async createAndSendInvite(name: string, email: string, baseUrl: string): Promise<IRegistrationInvitation> {
        try {
            const uniqueCode = await this.generateUniqueCode();
            const link = `${baseUrl}/sel-registration/${uniqueCode}`;

            const invite = new RegistrationInvitationModel({ name, email, link });
            await invite.save();
            ...
            try {
                ...
                throw new Error("Failed to send the registration email. Please try again.");
            }
     ...
     */
    private async sendEmail(name: string, email: string, fullLink: string): Promise<void> {
        try {
            const mailOptions = {
     ...
     */
    async confirmInvitation(link: string): Promise<IRegistrationInvitation | null> {
        try {
            const [baseUrl] = link.split('/sel-registration');
            const hashedCode = link.replace(`${baseUrl}/sel-registration/`, "").replace(/-/g, "/"); // Reverse URL-safe transformation

            const invite = await RegistrationInvitationModel.findOne({ link });
            if (!invite) {
